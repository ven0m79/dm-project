...
const CatalogPage = ({ params: { locale } }: { params: { locale: string } }) => {
  ...
  return (
    <MainLayout>
      <div className={classNames("text-2xl flex flex-wrap justify-center mb-5 px-5 gap-5 mt-5", [styles.catalogContainer])}>
        <Link href="/catalog/sub-catalog"><div
        ...
        ><span className={styles.span}>Обладнання для відділень інтенсивної терапії</span>
          <Image
            className={styles.img}
            src={imgIntensive}
            width={130}
        ...
        <Link href={{ pathname: '/catalog/sub-catalog', query: "neonat" }}><div
          className={classNames(
            "flex flex-row items-center rounded-xl",
            styles["block-decisions"],
          )}
        ...
        ><span className={styles.span}>Стерилізаційне та дезінфекційне обладнання</span>
          <Image
            className={styles.img}
            src={imgSteriliz}
            width={130}
