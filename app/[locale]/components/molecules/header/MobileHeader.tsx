...
const MobileHeader: FC<{ searchTerm: string, loading: boolean, setSearchTerm: Dispatch<SetStateAction<string>>, products: Product[] }> = ({ searchTerm, setSearchTerm, loading, products }) => {
  ...
  // Обработчик фокусировки при переходе по ссылке
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.addEventListener("focusSearch", handleSearchFocus);
    }

    return () => {
      window.removeEventListener("focusSearch", handleSearchFocus);
    };
  }, []);
